...
export default function TemplateLibrary() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId) {
        await updateTemplate({ id: editingId, name: bName.trim(), icon: bIcon, blocks: newBlocks });
      } else {
        await saveTemplate({ name: bName.trim(), icon: bIcon, blocks: newBlocks });
      }
  ...
  async function handleShare(t: UserTemplate) {
    ...
    await shareTemplate(t.id, newVal);
    await load();
  }
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2">
          {list.map((t) => (
            <div key={t.id} className="flex items-center gap-2 group">
              <div className={`flex-1 p-3 rounded-xl border-2 transition ${t.is_owner ? "border-paper-line bg-paper-card" : "border-paper-line/60 bg-paper-surface/50"}`}>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-xl">{t.icon}</span>
                  <span className="font-medium text-sm text-paper-ink">{t.name}</span>
                  {t.is_public && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200">🌐 共享</span>}
                </div>
                <div className="text-[11px] text-paper-ink2 leading-tight flex items-center gap-2">
                  <span>{blocksSummary(t.blocks)}</span>
                  {!t.is_owner && t.author_name && <span className="text-paper-ink3">· 由 {t.author_name} 发布</span>}
                </div>
              </div>
              {t.is_owner && (
                <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition">
                  <button
                    title="编辑"
                    onClick={() => openEdit(t)}
                    className="w-8 h-8 rounded-lg bg-paper-surface border border-paper-line text-paper-ink2 hover:border-violet-300 hover:text-violet-500 hover:bg-violet-50 flex items-center justify-center"
                  ><Pencil size={13} /></button>
                  <button
                    title={t.is_public ? "取消共享" : "共享"}
                    onClick={() => handleShare(t)}
                    className={`w-8 h-8 rounded-lg border flex items-center justify-center transition ${t.is_public ? "bg-emerald-50 border-emerald-200 text-emerald-600" : "bg-paper-surface border-paper-line text-paper-ink2 hover:border-emerald-300 hover:text-emerald-500"}`}
                  ><Share2 size={13} /></button>
                  <button
                    title="删除"
                    onClick={() => handleDelete(t.id, t.name)}
                    className="w-8 h-8 rounded-lg bg-paper-surface border border-paper-line text-paper-ink2 hover:border-red-200 hover:text-red-500 hover:bg-red-50 flex items-center justify-center"
      ...
      {showBuilder && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => !saving && setShowBuilder(false)}>
          ...
          <div
            className="relative w-full max-w-md max-h-[85vh] bg-paper-card rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col"
                  ...
                  {["📋", "🎯", "💼", "🏃", "📖", "✈️", "🧘", "💡", "🎨", "🍱"].map((ic) => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center transition ${
                        bIcon === ic ? "bg-violet-100 border-2 border-violet-400" : "bg-paper-surface border border-paper-line"
                      }`}
                    >{ic}</button>
                  ))}
                </div>
                  ...
                  {AVAILABLE_COMPONENTS.map((comp) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setBKinds((prev) =>
                            prev.includes(comp.kind) ? prev.filter((k) => k !== comp.kind) : [...prev, comp.kind]
                          );
                        }}
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition text-left ${
                          checked ? "bg-violet-50 border-violet-400 text-violet-800" : "bg-paper-surface border-paper-line text-paper-ink"
                        }`}
                      >
