...
export default function EditorPage({ initialDiary, onSave, onDelete, onCancel }: Props) {
  ...
  }, [title, blocks, moodId, date, initialDiary]);
  ...
  return (
    <div className="min-h-screen bg-paper-bg flex flex-col">
      ...
      {/* 日期 + 心情 */}
      <div className="max-w-2xl w-full mx-auto px-4 pt-3 flex items-center gap-3">
        <input
          type="date"
          value={date}
          onChange={(e) => setDate(e.target.value)}
          className="text-sm text-paper-ink bg-paper-surface border border-paper-line rounded-lg px-2 py-1.5"
        />
        <button
          onClick={() => setShowMood((s) => !s)}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-paper-surface border border-paper-line text-sm"
        >
          {moodId ? (
      ...
      {showMood && (
        <div className="max-w-2xl w-full mx-auto px-4 pt-2 pb-3 flex gap-2 flex-wrap">
          {MOOD_TAGS.map((m) => (
            <button
              key={m.id}
      ...
      {/* 块列表 */}
      <div className="flex-1 max-w-2xl w-full mx-auto px-4 pb-24">
        {blocks.map((b, idx) => (
          <div key={b.id} className="block-enter">
            {b.kind === "text" && (
